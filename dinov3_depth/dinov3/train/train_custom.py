...
def get_args_parser(add_help: bool = True):
    ...
    parser.add_argument("--exp-name", type=str, default="", help="label for the experiment")
...
def build_data_loader_from_cfg(
    ...
    dataset_strings = dataset_path.split("+")  # split on '+'
    datasets = [
        make_dataset(
            dataset_str=ds_str,
...
def do_train(cfg, model, resume=False):
    ...
    for data in metric_logger.log_every(
        ...
        wandb.log({
            ...
            **{k: v.item() for k, v in metrics_dict.items()},
            "local_batch_size": data["local_batch_size"] if "local_batch_size" in data else cfg.train.batch_size_per_gpu,
