...
class DataAugmentationDINO(object):
    def __init__(
        ...
        self.normalize = v2.Compose(
            [
                ...
                make_normalize_transform(mean=[0.0], std=[1.0]),
            ]
        )
    ...
    def __call__(self, image):
        output = {}
        output["weak_flag"] = True  # residual

        # ensure tensor with shape (C,H,W)
        if isinstance(image, np.ndarray):
            ...
            if image.ndim == 3 and image.shape[0] != 1:
                image = image.mean(dim=0, keepdim=True)  # force single channel if needed
        else:
            raise TypeError(f"Unsupported image type: {type(image)}")

        # global crops (geometric only)
        im1_base = self.geometric_augmentation_global(image)
        global_crop_1_transf = im1_base
        global_crop_1 = self.resize_global_post_transf(global_crop_1_transf)

        im2_base = self.geometric_augmentation_global(image)
        global_crop_2_transf = im2_base
        global_crop_2 = self.resize_global_post_transf(global_crop_2_transf)

        output["global_crops"] = [global_crop_1, global_crop_2]

        # global crops for teacher (geometric only)
        output["global_crops_teacher"] = [global_crop_1, global_crop_2]

        # crops for gram teacher (geometric only)
